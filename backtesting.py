...
for stock in tickers:
  try:
    ...
    bt = Backtest(df, Candles,cash=1e7, commission=.000,exclusive_orders=False)
    output = bt.run()
...
print('All data')
print('Candles>B&H: ',format((sorted['BnH Return']<sorted['Candles']).sum()/sorted.shape[0], '.3f'))
...
print('Top 250')
print('Candles>B&H: ',format((sorted['BnH Return'].iloc[250:]<sorted['Candles'].iloc[250:]).sum()/sorted.iloc[250:].shape[0], '.3f'))
...
print('Bottom 100')
print('Candles>B&H: ',format((sorted['BnH Return'].iloc[:100]<sorted['Candles'].iloc[:100]).sum()/sorted.iloc[:100].shape[0], '.3f'))
...
print('Candles: ',format((sorted['Candles']>1).sum()/sorted.shape[0], '.3f'))
